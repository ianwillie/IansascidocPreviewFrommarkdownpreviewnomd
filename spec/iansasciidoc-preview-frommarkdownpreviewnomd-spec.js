...
describe('Markdown Preview', function () {
  ...
  const expectPreviewInSplitPane = function () {
    ...
    runs(() => {
      expect(preview).toBeInstanceOf(IansasciidocPreviewFrommarkdownpreviewnomdView)
    ...
    it('displays a markdown preview in a split pane', function () {
      ...
      runs(() =>
        atom.commands.dispatch(
          ...
          'iansasciidoc-preview-frommarkdownpreviewnomd:toggle'
        )
      )
    ...
    describe('when the editor is modified', function () {
      ...
      describe('when the liveUpdate config is set to false', function () {
        it('only re-renders the markdown when the editor is saved, not when the contents are modified', function () {
          atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.liveUpdate', false)
  ...
  describe('when the markdown preview view is requested by file URI', function () {
    it('opens a preview editor and watches the file for changes', function () {
      waitsForPromise('atom.workspace.open promise to be resolved', () =>
        atom.workspace.open(
          `iansasciidoc-preview-frommarkdownpreviewnomd://${atom.project
            .getDirectories()[0]
  ...
  describe("when the editor's grammar it not enabled for preview", function () {
    it('does not open the markdown preview', function () {
      atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.grammars', [])
  ...
  describe('when the URI opened does not have a iansasciidoc-preview-frommarkdownpreviewnomd protocol', function () {
  ...
  describe('iansasciidoc-preview-frommarkdownpreviewnomd:toggle', function () {
    beforeEach(() =>
      waitsForPromise(() => atom.workspace.open('code-block.md'))
    )

    it('does not exist for text editors that are not set to a grammar defined in `iansasciidoc-preview-frommarkdownpreviewnomd.grammars`', function () {
      atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.grammars', ['source.weird-md'])
      ...
      expect(commands).not.toContain('iansasciidoc-preview-frommarkdownpreviewnomd:toggle')
    })

    it('exists for text editors that are set to a grammar defined in `iansasciidoc-preview-frommarkdownpreviewnomd.grammars`', function () {
      atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.grammars', ['source.gfm'])
      const editorElement = atom.workspace.getActiveTextEditor().getElement()
      const commands = atom.commands
        .findCommands({ target: editorElement })
        .map(command => command.name)
      expect(commands).toContain('iansasciidoc-preview-frommarkdownpreviewnomd:toggle')
    })

    it('updates whenever the list of grammars changes', function () {
      // Last two tests combined
      atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.grammars', ['source.gfm', 'text.plain'])
      const editorElement = atom.workspace.getActiveTextEditor().getElement()
  ...
  describe('when iansasciidoc-preview-frommarkdownpreviewnomd:copy-html is triggered', function () {
...
`)
  ...
  describe('GitHub style markdown preview', function () {
    beforeEach(() => atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.useGitHubStyle', false))
    ...
    it('renders markdown using the GitHub styling when enabled', function () {
      atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.useGitHubStyle', true)
    ...
    it('updates the rendering style immediately when the configuration is changed', function () {
      ...
      runs(() => {
        ...
        atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.useGitHubStyle', false)
        expect(preview.element.getAttribute('data-use-github-style')).toBeNull()
      })
    })
  })

  describe('when iansasciidoc-preview-frommarkdownpreviewnomd:save-as-html is triggered', function () {
    beforeEach(function () {
