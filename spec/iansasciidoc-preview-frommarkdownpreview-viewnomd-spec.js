...
const IansasciidocPreviewFrommarkdownpreviewnomdView = require('../lib/iansasciidoc-preview-frommarkdownpreviewnomd-view')
const TextMateLanguageMode = new TextEditor().getBuffer().getLanguageMode()
  .constructor

describe('IansasciidocPreviewFrommarkdownpreviewnomdView', function () {
  let preview = null
  ...
  beforeEach(function () {
    ...
    preview = new IansasciidocPreviewFrommarkdownpreviewnomdView({ filePath })
    ...
    waitsForPromise(() => atom.packages.activatePackage('iansasciidoc-preview-frommarkdownpreviewnomd'))
  })
  ...
  describe('serialization', function () {
    ...
    it('does not recreate a preview when the file no longer exists', function () {
      const filePath = path.join(temp.mkdirSync('iansasciidoc-preview-frommarkdownpreviewnomd-'), 'foo.md')
    ...
    it('serializes the editor id when opened for an editor', function () {
      ...
      runs(function () {
        preview = new IansasciidocPreviewFrommarkdownpreviewnomdView({
...
`)
  ...
  describe('image resolving', function () {
    ...
    describe('when the image uses an absolute path that exists', function () {
      it("doesn't change the URL when allowUnsafeProtocols is true", function () {
        ...
        atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.allowUnsafeProtocols', true)
    ...
    it('removes the URL when allowUnsafeProtocols is false', function () {
      ...
      atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.allowUnsafeProtocols', false)
  ...
  describe('gfm newlines', function () {
    describe('when gfm newlines are not enabled', function () {
      it('creates a single paragraph with <br>', function () {
        atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.breakOnSingleNewline', false)
    ...
    describe('when gfm newlines are enabled', function () {
      it('creates a single paragraph with no <br>', function () {
        atom.config.set('iansasciidoc-preview-frommarkdownpreviewnomd.breakOnSingleNewline', true)
  ...
  describe('when core:save-as is triggered', function () {
    ...
    it('saves the rendered HTML and opens it', function () {
      ...
      const IansasciidocPreviewFrommarkdownpreviewnomdStyles = [
        {
          rules: [createRule('.iansasciidoc-preview-frommarkdownpreviewnomd', '{ color: orange; }')]
        },
        {
          rules: [
            createRule('.not-included', '{ color: green; }'),
            createRule('.iansasciidoc-preview-frommarkdownpreviewnomd :host', '{ color: purple; }')
          ]
        }
      ]
      ...
      const atomTextEditorStyles = [
        ...
        'atom-text-editor .hr { background: url(atom://iansasciidoc-preview-frommarkdownpreviewnomd/assets/hr.png); }'
      ]
      ...
      runs(() => {
        ...
        spyOn(preview, 'getDocumentStyleSheets').andReturn(
          IansasciidocPreviewFrommarkdownpreviewnomdStyles
        )
...
`)
  ...
  describe('when iansasciidoc-preview-frommarkdownpreviewnomd:select-all is triggered', function () {
    it('selects the entire Markdown preview', function () {
      const filePath = atom.project
        .getDirectories()[0]
        .resolve('subdir/code-block.md')
      const preview2 = new IansasciidocPreviewFrommarkdownpreviewnomdView({ filePath })
      jasmine.attachToDOM(preview2.element)

      waitsForPromise(() => preview.renderMarkdown())

      runs(function () {
        atom.commands.dispatch(preview.element, 'iansasciidoc-preview-frommarkdownpreviewnomd:select-all')
      ...
      runs(() => {
        atom.commands.dispatch(preview2.element, 'iansasciidoc-preview-frommarkdownpreviewnomd:select-all')
        const selection = window.getSelection()
  ...
  describe('when iansasciidoc-preview-frommarkdownpreviewnomd:zoom-in or iansasciidoc-preview-frommarkdownpreviewnomd:zoom-out are triggered', function () {
    it('increases or decreases the zoom level of the markdown preview element', function () {
      ...
      runs(function () {
        ...
        atom.commands.dispatch(preview.element, 'iansasciidoc-preview-frommarkdownpreviewnomd:zoom-in')
        expect(getComputedStyle(preview.element).zoom).toBeGreaterThan(
          originalZoomLevel
        )
        atom.commands.dispatch(preview.element, 'iansasciidoc-preview-frommarkdownpreviewnomd:zoom-out')
