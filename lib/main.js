...
import  { renderAsciidoctor } from "/media/AcerWinData/github_from_home/IansascidocPreviewFrommarkdownpreviewnomd/lib/renderer.js";
...
module.exports = {
  ...
  // called wi ctrl-alt-shift-F  NB ctrl-alt-shift-B is used by JavaScript copied to clipboard too at end commented our for now
    ...
    //  textToHTML = Asciidoctor.convert(text,   { 'safe': 'server',   'attributes': { 'showtitle': true }}); // ,  WORKS
      ...
      //const activePaneItem = atom.workspace.getActivePaneItem()
      const filePath = 'atomClipboard.html' //atom.workspace.getActivePane().saveItemAs(activePaneItem); //WORKS but asks to save as ...adoc
    console.log(`OBP filePath = ${filePath}`);
      ...
      fs.writeFileSync( path.join( 'atomClipboard.html'), String(wordsHTML))
      ;
